            ...
            <div class="card">
                <div class="card-body d-flex flex-column gap-2">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        v-if="detailOrder.status == 'pending'" class="btn btn-primary btn-sm text-sm">Bayar Sekarang</button>
                    <button v-else-if="detailOrder.status == 'success'"
                        class="btn btn-success btn-sm">{{ detailOrder.status }}</button>
                    <button v-else-if="detailOrder.status == 'expired'"
                        class="btn btn-warning btn-sm">{{ detailOrder.status }}</button>
                    <button v-else-if="detailOrder.status == 'failed'"
                        class="btn btn-danger btn-sm">{{ detailOrder.status }}</button>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body d-flex flex-column gap-2">
                    ...
                    <div class="d-flex justify-content-between align-items-center" v-for="p in detailOrder.orders">
                        <div class="d-flex gap-2">
                            <img :src="`${Url()}/products/${p.product.image}`" :alt="p.product.slug" class="img-fluid rounded">
            ...
            <div class="card">
                <div class="card-body d-flex flex-column gap-2">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <p class="m-0 fw-bold">Rp.{{moneyFormat((detailOrder.grand_total * 10/100) + detailOrder.grand_total)}}</p>
                    </div>
                </div>
            </div>
